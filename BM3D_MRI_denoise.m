function [PSNR, y_est] = BM3D_MRI_denoise(y, z, sigma, profile, print_to_screen)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
%% The original of this file is "BM3D.m". In this file the secondary Wiener filtering step has been cancelled. 
%%
...
N1                  = 8;%8;   %% N1 x N1 is the block size used for the hard-thresholding (HT) filtering
...
Ns                  = 19; %39;  %% length of the side of the search neighborhood for full-search block-matching (BM), must be odd
tau_match           = 3000; %3000;%% threshold for the block-distance (d-distance)
...
beta                = 2.0;%2.0; %% parameter of the 2D Kaiser window used in the reconstruction
...
%%%%
lambda_thr3D_change=1;
%tic;
y_hat = bm3d_thr(z, hadper_trans_single_den, Nstep, N1, N2, lambda_thr2D,...
lambda_thr3D_change, tau_match*N1*N1/(255*255), (Ns-1)/2, (sigma/(255)), thrToIncStep, single(Tfor), single(Tinv)', inverse_hadper_trans_single_den, single(thr_mask), Wwin2D, smallLN, stepFS );
